...
const cacheReservedDates = (req, res, next) => {
  ...
  const key = `${restaurantID}-${dateTime}-${partySize}`;
  console.log('key in cache function', key);
  client.get(key, (err, data) => {
    if (err) throw err;

    if (data !== null) {
      console.log('caching works! sending data...');
        ...
        pool.query('SELECT date_time, count(*) FROM reservations WHERE restaurant_id = $1 AND date_time >= $2 AND date_time <= $3 AND table_size = $4 GROUP BY date_time',
          [restaurantID, start, end, tableSize], (err, rezData) => {
            if (err) {
              ...
              console.log('key in actual function', key);
              client.set(key, JSON.stringify(reserved));
              res.send(reserved);
            }
          });
      }
